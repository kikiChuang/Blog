...
<script type="text/javascript">
    ...
    function showOtherComment() {
    	$('.otherComment').removeClass('otherComment').addClass('comment');
    }
			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="keyWord" items="${keyWords }">
						&nbsp;&nbsp;<a href="${pageContext.request.contextPath }/blog/search.html?q=${keyWord }" target="_blank">${keyWord }</a>&nbsp;&nbsp;
					</c:forEach>
				</c:otherwise>
			</c:choose>
		</div>
				...
				<c:forEach var="comment" items="${commentList }" varStatus="status">
					<c:choose>
						<c:when test="${status.count<=10 }">
							<div class="comment">
								<span><font>${status.count }楼&nbsp;&nbsp;${comment.userIp }：</font>${comment.content }&nbsp;&nbsp;[&nbsp;<fmt:formatDate value="${comment.commentDate }" type="date" pattern="yyyy-MM-dd HH:mm"/>&nbsp;]</span>
							</div>
						</c:when>
						<c:otherwise>
...
<div class="data_list" >
	...
	<div class="publish_comment">
		...
		<div class="verCode">
			验证码：<input type="text" name="imageCode" id="imageCode" size="10" onkeydown="if(event.keyCode==13) submitData()"/>&nbsp;<img onclick="loadimage()" title="换一张试试" name="randImage" id="randImage" src="${pageContext.request.contextPath }/image.jsp" width="60" height="20" border="1" align="absmiddle"/> 
		</div>
