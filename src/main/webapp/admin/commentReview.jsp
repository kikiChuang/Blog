...
<script type="text/javascript">
	function formatBlogTitle(val,row) {
		...
		return "<a href='${pageContext.request.contextPath}/blog/detail/"+val.id+".html' target='_blank'>"+val.title+"</a>";
	}
	...
	function commentReview(state) {
		...
		$.messager.confirm("系统提示","您确定要审核这<font color='red'>"+selectedRows.length+"</font>条评论吗？", function(r){
			if(r) {
				$.post("${pageContext.request.contextPath}/admin/comment/review.do",{"idStr":idStr,"state":state},function(result){
					if(result.success) {
...
<table id="dg" title="评论审核" class="easyui-datagrid" fitColumns="true" pagination="true" rownumbers="true" url="${pageContext.request.contextPath }/admin/comment/list.do?state=0" fit="true" toolbar="#tb">
	<thead>
		<tr>
