...
<script type="text/javascript">
	...
	function openBlogTypeModifyDialog() {
		...
		if(selectedRows.length!=1) {
			$.messager.alert("系统提示","请选择一条要修改的博客类别！");
			return;
		}
	...
	function deleteBlogType() {
		...
		if(selectedRows.length==0) {
			$.messager.alert("系统提示","请选择要删除的博客类别！");
			return;
		}
		var idArr = [];
		...
		$.messager.confirm("系统提示","您确定要删除这<font color='red'>"+selectedRows.length+"</font>条博客类别吗？", function(r){
			if(r) {
				$.post("${pageContext.request.contextPath}/admin/blogType/delete.do",{"idStr":idStr},function(result){
					if(result.success) {
						if(result.exist) {
							$.messager.alert("系统提示","博客类别下有博客，不能删除！");
						} else {
							$.messager.alert("系统提示","博客类别已成功删除！");							
						}
						$("#dg").datagrid("reload");
					} else {
						$.messager.alert("系统提示","博客类别删除失败！");
					}
				},"json");
			}
		});
...
<table id="dg" title="博客类别管理" class="easyui-datagrid" fitColumns="true" pagination="true" rownumbers="true" url="${pageContext.request.contextPath }/admin/blogType/list.do" fit="true" toolbar="#tb">
	<thead>
		<tr>
	...
	<form id="fm" method="post">
		<table cellspacing="8px">
			...
			<tr>
				<td>博客类别排序：</td>
				<td>
					<input type="text" id="orderNo" name="orderNo" class="easyui-numberbox" required="true" style="width: 60px;"/>&nbsp;
					(类别根据排序序号从小到大排序)
				</td>
			</tr>
		</table>
